...
// ファイル指定位置をメモリに読み込む 
// 読み込むキャッシュインデックス
void* Mapper::map( unsigned int offsetIdx ) // mapping file point
{
	
	
	ftruncate(  _fd , offsetIdx * pow(2, 8*2) ); // ファイルの拡張
	...
	long totalByteCount = offsetIdx * pow(2, 2*8); // 最低確保しなければならないファイルサイズ 最適化していないためframeまでのファイルを確保する必要がある

	if( totalByteCount > currentFileSize )
		if( ftruncate( _fd , totalByteCount ) != 0 ) return nullptr; // 新たにファイルに領域を確保する

	if( ( offsetIdx * DEFAULT_1FRAME_CACHE_SIZE ) % _systemPageSize != 0 ){
		std::cerr << " [ Important Warning ] マッピングするファイルブロックがシステムのページサイズの倍数ではありません" << "\n";
		return nullptr;
	}
	
	void *mappedPtr = nullptr; 
	mappedPtr = mmap( NULL , DEFAULT_1FRAME_CACHE_SIZE  , PROT_READ | PROT_WRITE , MAP_SHARED , _fd , ( offsetIdx * DEFAULT_1FRAME_CACHE_SIZE ) );	
...
{
	// エラー発生したらどうするか
	return munmap( targetMappedPtr , DEFAULT_1FRAME_CACHE_SIZE );
}


int Mapper::sync( void* targetMappedPtr )
{
	return msync( targetMappedPtr , DEFAULT_1FRAME_CACHE_SIZE , MS_SYNC );
}
