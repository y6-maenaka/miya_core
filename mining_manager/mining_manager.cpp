...
mining_manager::minig_control_block::minig_control_block( io_context &io_ctx, struct block_header block_header ) : 
...
bool mining_manager::minig_control_block::invalid( const mb_id &id )
{
  boost::uuids::uuid invalid_id; std::fill( invalid_id, '0xff' );
...
mining_manager::mb_id mining_manager::mine_block( struct chain::block_header target, std::function<void(void)> on_success_mine, std::function<void(void)> on_failure_mine, unsigned short launch_thread_count, std::time_t timeout )
{
  ...
  for( int i=0; i<launch_thread_count; i++ ){
	struct mining_manager::mining_conrtol_block::segment sgm;
	boost::thread mining_th( mine_thread_func, std::ref( sgm ) );
