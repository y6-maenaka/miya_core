...
std::shared_ptr<struct BDBCB> BDFilter::filter( std::shared_ptr<Block> target , bool isReject )
{
  if( _isClose ) return nullptr;
...
std::shared_ptr<struct BDBCB> BDFilter::filter( std::shared_ptr<BlockHeader> target , bool isReject )
{
  if( _isClose ) return nullptr;
...
void BDFilter::mask( std::vector< std::shared_ptr<BlockHeader>> targetVector )
{
