...
private:
	...
	std::unordered_map< struct BlockHashAsKey, std::shared_ptr<struct BDBCB> , BlockHashAsKey::Hash > _filter;
public:
	BDFilter();

	std::shared_ptr<struct BDBCB> filter( std::shared_ptr<unsigned char> target, bool isReject = false );
	std::shared_ptr<struct BDBCB> filter( std::shared_ptr<Block> target, bool isReject = false );
	std::shared_ptr<struct BDBCB> filter( std::shared_ptr<BlockHeader> target, bool isReject = false );

	void mask( std::vector< std::shared_ptr<BlockHeader>> targetVector ); // かなり重い処理
