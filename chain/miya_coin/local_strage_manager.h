...
public:
	BlockContainer();
	BlockContainer( std::shared_ptr<Block> block );
...
public:
	...
	std::vector< std::shared_ptr<UTXO> > _utxoVector;

public:
	UndoContainer();
	UndoContainer( std::shared_ptr<Block> blockFrom ); // このブロック内のトランザクションからundoを作成する
...
public:
	...
	long write( std::shared_ptr<Block> targetBlock );
...
public:
	...
	std::shared_ptr<struct Block> readBlock( std::shared_ptr<unsigned char> blockHash );
