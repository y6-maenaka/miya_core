...
int virtual_chain_p5()
{
  ...
  std::shared_ptr<miya_chain::BDFilter> filter = std::shared_ptr<miya_chain::BDFilter>( new miya_chain::BDFilter() );
  miya_chain::FilterStateUpdator filterStateUpdator = std::bind(
													 &miya_chain::BDFilter::update ,
													 std::ref(filter),
													std::placeholders::_1,
													std::placeholders::_2
													);
  ...
  std::shared_ptr<miya_chain::VirtualHeaderSyncManager> headerSyncManager = std::shared_ptr<miya_chain::VirtualHeaderSyncManager>( 
			new miya_chain::VirtualHeaderSyncManager(
			block_0000->header(),
			nullptr,
			filterStateUpdator,
			toRequesterSBC
		));
  headerSyncManager->start(); 
